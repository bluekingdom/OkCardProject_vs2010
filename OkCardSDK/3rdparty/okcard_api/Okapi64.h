...
#else
	typedef		long long			MLONG,	*PMLONG, *LPMLONG;	//which are 4 bytes in 32 system, 8 bytes in 64 system 
	typedef		unsigned long long	MWORD,	*PMWORD, *LPMWORD;  //which are 4 bytes in 32 system, 8 bytes in 64 system 
...
#define		VIDEO_SOURCECHAN		1 //lParam=0,1.. Comp.Video; 0x100,101...to Y/C(S-Video), 0x200,0x201 to RGB Chan.Input
...
//--set params to out to the other device
#define		VIDEO_OUTLINEPERIOD		14 //out line period (in 0.54 us) generated by board
#define		VIDEO_LINEPERIOD		14 //HIWORD==1: in 0.1 vs, ==2: ...
...
#define		CAPTURE_FRAMERATE		22 //for camera, set frame rate to capture 
...
#define		CAPTURE_GAMMAFACTOR		30 //for camera, set gamma factor (it is between 0~2 for some camera)
...
//-----sub-function defines for wParam of SetDeviceParam
		//wParam cab be one of the follow
#define		DEVICE_SETDEVMEMORY		0 //set or get all parameters to and from device with memorory, 
									  //lParam=-1: get device's current params to pc, lParam=0: store PC's to device as current, 
...
#define		AUDIO_CALLINTERVAL		4 //min. interval times of callback 


//prolog and epilog
HANDLE	WINAPI okOpenAudio(HANDLE hBoard, MLONG lParam);
		//open audio device owned by the video capture board 
		//hBoard is handle of image board, lParam reserved argument must be set to 0
